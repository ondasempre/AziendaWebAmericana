...
case $1 in
   f|fullcone)
      ...
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 10.1.0.2 --jump SNAT --to-source 193.204.161.2
      
      iptables --table nat --append PREROUTING --in-interface eth1 --destination 193.204.161.2 --jump DNAT --to-destination 10.1.0.2
      ;;
   ...
   r|restricted)
      ...
      iptables --append FORWARD --source 10.1.0.0/24 --out-interface eth1 --jump ULOG
      ...
      cat /var/log/ulog/syslogemu.log | awk -W interactive '
         {
            for (i=1; i<=NF; i++) {
               ...
               if (fields[1]=="SRC") {
                  src=fields[2]
                  if (src=="10.1.0.2") { ext_src="193.204.161.2"; }
                  if (src=="10.1.0.3") { ext_src="193.204.161.15"; }
               }
            }
   ...
   s|symmetric)
      # Appear to the outside with fixed addresses
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 10.1.0.2 --destination 193.204.161.1 --jump SNAT --to-source 193.204.161.2
